...
class CmDateUtil {
    ...
    constructor(opt = {}) {
        ...
        const { locale = resolvedOptions.locale, timeZone = resolvedOptions.timeZone } = opt;
     ...
     */
    getElapsedTime(activeDate, passiveDate = new Date(), timeZone = this.timeZone) {
        const remainMonth = this.toDayjs(activeDate, timeZone).diff(passiveDate, 'month');
        let diffMonth = 0;
        if (remainMonth > 0) {
            diffMonth = remainMonth;
            activeDate = this.toDayjs(activeDate, timeZone).subtract(diffMonth, 'month');
        }
        let remainMs = this.toDayjs(activeDate, timeZone).diff(passiveDate, 'millisecond');
    ...
    #getElapsedTimeRtfConfig(activeDate, option = {}) {
        const { locale = this.locale, passiveDate = new Date(), timeZone = this.timeZone } = option;
    ...
    toFormatRtf(activeDate, option = {}) {
        ...
        const { locale, unit, value } = this.#getElapsedTimeRtfConfig(activeDate, option);
    ...
    toFormatDtf(dateValue, option = {}) {
        const { flag = 'base', locale = this.locale, timeZone = this.timeZone } = option;
    ...
    toFormat(activeDate, option = {}) {
        ...
        const isToday = this.toDayjs(activeDate, timeZone).isSame(new Date(), 'day');
     ...
     */
    toStrRange(dateValue, resetOf = 'day', formatUnit = 'day', hasEndDate = false) {
        const allowUnits = ['millisecond', 'second', 'minute', 'hour', 'day', 'month', 'year'];
        ...
        const endDateValue = Array.isArray(dateValue) ? dateValue[dateValue.length - 1] : dateValue;
        ...
        return [this.toStrFormat(startDayjs, formatUnit), this.toStrFormat(endDayjs, formatUnit)];
    }
}

module.exports = CmDateUtil;
